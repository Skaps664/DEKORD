...
export default function InfiniteGallery({
  ...
  useEffect(() => {
    // Scope wheel handler to this gallery's own canvas (not the first canvas in the document)
    const canvas = containerRef.current?.querySelector("canvas")
    if (canvas) {
      const wheelHandler = (event: WheelEvent) => {
        event.preventDefault()
        setScrollVelocity((prev) => prev + event.deltaY * 0.01 * fadeSettings.fadeIn.end)
        setAutoPlay(false)
      }
      const keyHandler = (event: KeyboardEvent) => {
        if (event.key === "ArrowUp" || event.key === "ArrowLeft") {
          setScrollVelocity((prev) => prev - 2 * fadeSettings.fadeIn.end)
          setAutoPlay(false)
        } else if (event.key === "ArrowDown" || event.key === "ArrowRight") {
          setScrollVelocity((prev) => prev + 2 * fadeSettings.fadeIn.end)
          setAutoPlay(false)
      ...
      // Only capture keyboard when the gallery is focused/hovered
      canvas.addEventListener("mouseenter", () => document.addEventListener("keydown", keyHandler))
      canvas.addEventListener("mouseleave", () => document.removeEventListener("keydown", keyHandler))

      return () => {
        canvas.removeEventListener("wheel", wheelHandler)
        canvas.removeEventListener("mouseenter", () => document.addEventListener("keydown", keyHandler))
        canvas.removeEventListener("mouseleave", () => document.removeEventListener("keydown", keyHandler))
        document.removeEventListener("keydown", keyHandler)
      }
    }
  }, [fadeSettings.fadeIn.end])
  ...
  return (
    <div
      ...
      style={style}
      role="region"
      aria-label="Interactive 3D image gallery. Scroll inside the box to navigate images."
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center text-center px-3">
        <h1 className="font-serif text-4xl md:text-7xl tracking-tight text-foreground">
          <span className="italic">I create;</span> therefore I am
        </h1>
      </div>
      <div className="pointer-events-none absolute bottom-5 left-0 right-0 text-center font-mono uppercase text-[11px] font-semibold text-foreground/90">
        <p>Scroll inside to explore • Scroll outside to navigate the page</p>
        <p className="opacity-60">Mouse wheel, arrow keys, or touch • Auto-play after 3s</p>
      </div>
    </div>
  )
}
