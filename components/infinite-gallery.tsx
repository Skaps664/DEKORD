...
function GalleryScene({
  ...
  isHovered = false,
}: Omit<InfiniteGalleryProps, "className" | "style" | "speed"> & { isHovered?: boolean }) {
  ...
  useFrame((state, delta) => {
    // Apply auto-play based on hover state
    const baseSpeed = 0.3 * delta
    const direction = isHovered ? -1 : 1 // Reverse when hovered
    setScrollVelocity((prev) => prev + baseSpeed * direction)
  ...
  return (
    ...
    >
      <Canvas camera={{ position: [0, 0, 0], fov: 55 }} gl={{ antialias: true, alpha: true }}>
        <GalleryScene images={images} fadeSettings={fadeSettings} blurSettings={blurSettings} isHovered={isHovered} />
      </Canvas>
